...
export default function SettingsClient({ initialData }: { initialData: Record<string, unknown> }) {
...
function ToggleField({ label, checked, onChange, description }: { label: string; checked: boolean; onChange: (v: boolean) => void; description?: string }) {
  return (
    <div className="flex items-start gap-3 p-3 bg-stone-50 rounded-lg">
      <button type="button" onClick={() => onChange(!checked)} className={`mt-0.5 w-10 h-5 rounded-full p-0.5 transition-colors flex-shrink-0 ${checked ? 'bg-amber-500' : 'bg-stone-300'}`}>
