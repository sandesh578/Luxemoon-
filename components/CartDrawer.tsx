'use client';
import { useCart, useLocationContext, useConfig, useI18n } from './Providers';
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in-right">
        <div className="p-5 border-b flex justify-between items-center bg-stone-50">
          <h2 className="font-serif text-xl font-bold text-stone-900">{t('cart.yourBag')} ({items.length})</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
              ...
              <p className="text-stone-500 font-medium">{t('cart.emptyBag')}</p>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                  <div>
                    ...
                    {item.stock < 5 && item.stock > 0 && (
                      <p className="text-[10px] font-bold text-red-600 mt-0.5">{t('cart.onlyLeft', { count: item.stock })}</p>
                    )}
                  </div>
        ...
        {items.length > 0 && (
          <div className="p-6 bg-stone-50 border-t border-stone-100 space-y-4">
            ...
            >
              {t('cart.checkoutNow').toUpperCase()}
            </Link>
          </div>
        )}
      </div>
